...
export function createCompiler({
    Args, commands, instructions, registers, keywords, memorySize, byteAddress, trackLineOffsets
}) {
    ...
    class Compiler {
        ...
        parseValue(resolveCallback, required = false) {
            ...
            if (token.type === Token.NAME) {
                ...
                resolveCallback(byteAddress(this.statementAddress));
        ...
        compile() {
            ...
            while ((token = this.tokenizer.next()).type !== Token.EOF)
                if (token.type === Token.INSTRUCTION) {
                    ...
                    if (token.type === Token.KEYWORD && token.value === "db") {
                        this.resolveReference(name, byteAddress(this.bytes.length));
                        do {
            ...
            if (this.bytes.length > memorySize)
